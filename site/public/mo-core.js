...
/* MoCreative Concept. — shared behaviour (theme, feather cursor, glass field).
   Framework-free, one file, used by every MoCreative build.

   1. THEME. data-theme on <html>: dark or light. The first choice comes from
      the OS (set before paint by an inline head snippet). The toggle stores the
      visitor's choice in localStorage, and failing to store is harmless.
   2. FEATHER. The owner's pen-and-feather image is the pointer on
      fine-pointer devices. It follows on a damped spring (a little overshoot,
      then it settles), leans with horizontal speed, floats slightly while idle,
      and the ink bead at the nib swells over anything pressable.
   3. GLASS. An original fluted-glass field fixed under the page: vertical ribs
      refract a soft graphite field with a warm light that follows the feather.
      Scrolling kicks a spring, so the ribs ripple and settle like struck glass.
      It is written for this brand, not taken from any other site. Rendered at
      reduced resolution, paused when hidden, and static under reduced motion.
*/
(function () {
  'use strict';
  ...
  function setTheme(t) {
    ...
    try { localStorage.setItem('mo-theme', t); } catch (e) {}
    document.querySelectorAll('[data-theme-toggle]').forEach(function (b) {
      b.setAttribute('aria-label', t === 'dark' ? 'Switch to light mode' : 'Switch to dark mode');
    });
    if (glass) glass.theme(t);
  }
  ...
  var pointer = { x: innerWidth * 0.5, y: innerHeight * 0.5, seen: false };
  if (fine && !reduced) {
    var pen = document.createElement('div');
    pen.className = 'mo-feather'; pen.setAttribute('aria-hidden', 'true');
    pen.innerHTML = '<span class="mo-feather__ink"></span>';
    document.body.appendChild(pen);
    root.classList.add('mo-cursor-on');

    var p = { x: pointer.x, y: pointer.y, vx: 0, vy: 0, tilt: 0 };
    var K = 0.2, D = 0.68;            // spring stiffness, damping: settles with a small overshoot
    var t0 = performance.now(), running = false;

    addEventListener('pointermove', function (e) {
      pointer.x = e.clientX; pointer.y = e.clientY;
      if (!pointer.seen) { pointer.seen = true; p.x = pointer.x; p.y = pointer.y; pen.classList.add('is-on'); }
      var hot = e.target.closest && e.target.closest('a, button, [role="button"], summary, label');
      pen.style.setProperty('--ink', hot ? '1.25' : '0.4');
      if (!running) { running = true; requestAnimationFrame(step); }
    }, { passive: true });
    addEventListener('pointerleave', function () { pen.classList.remove('is-on'); pointer.seen = false; });
    document.addEventListener('mouseleave', function () { pen.classList.remove('is-on'); pointer.seen = false; });
    addEventListener('pointerdown', function () { pen.style.setProperty('--ink', '1.6'); });

    function step(now) {
      p.vx = (p.vx + (pointer.x - p.x) * K) * D;
      p.vy = (p.vy + (pointer.y - p.y) * K) * D;
      p.x += p.vx; p.y += p.vy;
      var lean = Math.max(-18, Math.min(18, p.vx * 1.6));
      p.tilt += (lean - p.tilt) * 0.18;
      var idle = Math.sin((now - t0) / 900) * 1.6;       // a slow float, a couple of pixels
      pen.style.transform = 'translate3d(' + p.x.toFixed(1) + 'px,' + (p.y + idle).toFixed(1) + 'px,0) rotate(' + p.tilt.toFixed(2) + 'deg)';
      var moving = Math.abs(p.vx) + Math.abs(p.vy) > 0.02 || Math.abs(pointer.x - p.x) + Math.abs(pointer.y - p.y) > 0.3;
      if (moving || pointer.seen) requestAnimationFrame(step); else running = false;
    }
  }
  ...
  (function mountGlass() {
    ...
    canvas.className = 'mo-glass'; canvas.setAttribute('aria-hidden', 'true');
    ...
    var FS = [
      ...
      'uniform vec2 R; uniform float T; uniform float S; uniform float W; uniform vec2 M; uniform float L;',
      'uniform vec3 cBase; uniform vec3 cHi; uniform vec3 cGold;',
      'float h(vec2 p){p=fract(p*vec2(123.34,456.21));p+=dot(p,p+45.32);return fract(p.x*p.y);}',
      'float n(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);',
      ' return mix(mix(h(i),h(i+vec2(1,0)),f.x),mix(h(i+vec2(0,1)),h(i+vec2(1,1)),f.x),f.y);}',
      'float fbm(vec2 p){float v=0.,a=.5;for(int i=0;i<4;i++){v+=a*n(p);p=p*2.02+vec2(11.,7.);a*=.5;}return v;}',
      'void main(){',
      ' vec2 uv=gl_FragCoord.xy/R; float asp=R.x/R.y;',
      ' float ribs=clamp(R.x/58.,10.,30.);',            // flute width stays about 58px at any size
      ' float x=uv.x*ribs; float id=floor(x); float f=fract(x)-.5;',
      // the spring: struck-glass ripple travelling down each flute, amplitude W
      ' float wob=W*sin(uv.y*9.-T*3.2+id*.9)*.028;',
      ' vec2 q=uv+vec2(f*.075+wob,0.);',
      ' float fl=fbm(vec2(q.x*2.2,q.y*1.3-S*.35+T*.012));',
      ' vec2 d=(q-M)*vec2(asp,1.); float lit=exp(-dot(d,d)*3.4);',
      ' vec3 col=mix(cBase,cHi,smoothstep(.15,.95,fl));',
      ' col+=cGold*lit*(.5+.5*fl)*mix(.26,.16,L);',
      ' float bevel=cos(f*6.2832);',                       // each flute is lit on one side, shaded on the other
      ' col*=.965+.035*bevel; col+=cHi*.03*pow(1.-abs(f*2.),10.);',
      ' col+=cGold*.03*lit*smoothstep(.35,.5,abs(f));',
      ' float v=length((uv-.5)*vec2(1.1,1.));',
      ' col*=mix(1.,.82,smoothstep(.45,1.,v)*(1.-L*.8));',
    ...
    var U = {}; ['R', 'T', 'S', 'W', 'M', 'L', 'cBase', 'cHi', 'cGold'].forEach(function (k) { U[k] = gl.getUniformLocation(pr, k); });

    var PAL = {
      dark:  { base: [0.078, 0.071, 0.063], hi: [0.17, 0.155, 0.135], gold: [0.89, 0.66, 0.31], L: 0 },
      light: { base: [0.925, 0.894, 0.827], hi: [0.953, 0.929, 0.878], gold: [0.86, 0.62, 0.26], L: 1 }
    };
    var pal = PAL.dark, scale = 0.5;
    function applyTheme(t) {
      pal = PAL[t] || PAL.dark;
      gl.uniform3fv(U.cBase, pal.base); gl.uniform3fv(U.cHi, pal.hi); gl.uniform3fv(U.cGold, pal.gold); gl.uniform1f(U.L, pal.L);
    ...
    // spring state: scroll velocity kicks it; it rings and settles
    var sp = { x: 0, v: 0 }, lastY = scrollY, mx = 0.72, my = 0.62, raf = 0, visible = true, lastT = 0;
    function draw(now) {
      gl.uniform1f(U.T, now / 1000);
      gl.uniform1f(U.S, scrollY / innerHeight);
      gl.uniform1f(U.W, sp.x);
    ...
    function frame(now) {
      ...
      sp.v += Math.max(-2.5, Math.min(2.5, dy * 0.02));      // the kick
      sp.v += -0.06 * sp.x; sp.v *= 0.9; sp.x += sp.v;         // stiffness, damping
      sp.x = Math.max(-2.2, Math.min(2.2, sp.x));
      var tx = pointer.seen ? pointer.x / innerWidth : 0.72, ty = pointer.seen ? 1 - pointer.y / innerHeight : 0.62;
      mx += (tx - mx) * 0.06; my += (ty - my) * 0.06;
      if (now - lastT > 30) { draw(now); lastT = now; }         // about 30fps is plenty for a soft field
      raf = requestAnimationFrame(frame);
    }
    size(); applyTheme(root.getAttribute('data-theme') || 'dark');
    ...
    if (!reduced) raf = requestAnimationFrame(frame); else draw(1000);
    glass = { theme: applyTheme };
  })();

  // Sync labels with the theme set before paint, without storing it: until the
  // visitor chooses, the page keeps following the OS.
  (function (t) {
    document.querySelectorAll('[data-theme-toggle]').forEach(function (b) { b.setAttribute('aria-label', t === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'); });
    if (glass) glass.theme(t);
  })(root.getAttribute('data-theme') || 'dark');
  matchMedia('(prefers-color-scheme: light)').addEventListener('change', function (e) {
    var stored = null; try { stored = localStorage.getItem('mo-theme'); } catch (x) {}
    if (!stored) { var t = e.matches ? 'light' : 'dark'; root.setAttribute('data-theme', t); if (glass) glass.theme(t); }
  });
})();


/* ── ready: run queued page scripts, then run later ones immediately ── */
(function () {
  var q = window.__moReady || [];
  window.__moReady = { push: function (fn) { fn(); } };
